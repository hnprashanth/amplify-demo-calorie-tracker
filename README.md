...
Add following imports to 'src/App.js' at the top after uuid import statement
...
Let's update our logCalories function with code to call API
  ...
  logCalories = e => {
    ...
    let path = "/items";
    let data = {
      body: { user_id: this.state.user.attributes.sub, food_name, calories, created_at: Date.now() }
    };
    API.post(apiName, path, data)
      .then(response => {
        this.setState({
          entries: [
            ...this.state.entries,
            { id: this.state.user.attributes.sub, food_name, calories, created_at: Date.now() }
          ],
          food_name: "",
          calories: 0
        });
